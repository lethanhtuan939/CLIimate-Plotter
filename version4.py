...
def plot_data(cities, data_map, months_chosen, data_type):
    plt.figure(figsize=(12, 8))
    for city in cities:
        for data_label, data in data_map.items():
            city_data = data.loc[city, months_chosen]
            plt.plot(months_chosen, city_data, marker='o', label=f"{city.capitalize()}")
    ...
    while True:
        ...
        for city in cities_display:
            ...
            for data_type in data_display:
                if data_type == 'temp':
                    result = display_data(city, temp_data, months_display, "Temperature")
                elif data_type == 'rain':
                    result = display_data(city, rainfall_data, months_display, "Rainfall")
                elif data_type == 'sun':
                    result = display_data(city, sunshine_data, months_display, "Sunshine hours")
        ...
        if len(cities_display) > 0 and len(data_display) > 0:
            ...
            for data_type in data_display:
                plot_data(cities_display, {data_type_map[data_type]: data_map[data_type]}, months_display, data_type_map[data_type])
